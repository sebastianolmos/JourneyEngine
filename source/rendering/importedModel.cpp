...
namespace Journey {
    ...
    ImportedModel::ImportedModel(std::string const& path, bool gamma) : gammaCorrection(gamma)
    {
        loadModel(path);
    }

    void ImportedModel::drawCall(Shader& shader)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].Draw(shader);
    }

    void ImportedModel::loadModel(std::string const& path)
    {
    ...
    unsigned int TextureFromFile(const char* path, const std::string& directory, bool gamma)
    {
        ...
        //std::cout << "Directory requested " << filename.c_str() << std::endl;
